...
import { TrashIcon, PencilSquareIcon, PlusIcon, CheckCircleIcon, XCircleIcon } from "@heroicons/react/24/outline";
import ConfirmModal from "../../components/modals/ConfirmModal";

// --- Modal para crear/editar editorial ---
function EditorialModal({ trigger, editorial, onSaved, size = "sm" }) {
  const [data, setData] = useState(
    editorial || { name: "", active: true }
  );
  ...
  const handleSave = async (closeModal) => {
    ...
    try {
      ...
      if (editorial && editorial.id) {
        saved = await updateEditorial(editorial.id, data);
        toast.success("Editorial actualizada");
      } else {
        saved = await createEditorial(data);
  ...
  return (
    <Modal
      trigger={trigger}
      title={editorial ? "Editar editorial" : "Nueva editorial"}
      size={size}
      actions={[
        {
          label: editorial ? "Actualizar" : "Crear",
...
const EditorialPage = () => {
  ...
  const [processing, setProcessing] = useState(null); // ID de editorial en acción
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-blue-50 py-10 shadow-slate-300 shadow-2xl rounded-4xl">
      <div className="max-w-2xl mx-auto px-4">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Editoriales</h1>
          <EditorialModal
            trigger={
          ...
          <table className="min-w-full table-auto text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                editorials.map((ed) => (
                  <tr key={ed.id} className="border-t hover:bg-slate-50 transition">
                    <td className="py-2 px-4">{ed.id}</td>
                    <td className="py-2 px-4">{ed.name}</td>
                      ...
                      <ConfirmModal
                        trigger={
                          <button
                            ...
                            title="Eliminar"
                            disabled={processing === ed.id}
                          >
                            <TrashIcon className="w-5 h-5" />
                          </button>
                        }
                        title="¿Eliminar editorial?"
                        message={`¿Seguro que deseas eliminar "${ed.name}"?`}
                        confirmLabel="Eliminar"
                        cancelLabel="Cancelar"
                        size="sm"
                        confirmClassName="bg-red-600 text-white hover:bg-red-700"
                        onConfirm={() => handleDelete(ed)}
                      />
                    </td>
                  </tr>
                ))
